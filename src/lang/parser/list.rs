...
/// Parses a list (given that the `LBracket` has already been consumed)
pub fn parse_list(tokens: &mut SpannedIter<'_, Token>) -> Result<Spanned<Vec<Expr>>, Vec<KError<Error>>> {
    ...
    while let Some((token, span)) = next_tok {
        match token {
            Token::RBracket => return Ok((exprs, start_span.start..span.end)), // when the tuple is terminated
            Token::Comma => {
                match tokens.next() {
                    Some((Ok(Token::RBracket), span)) => return Ok((exprs, start_span.start..span.end)),
                    Some((Err(err), span)) => return Err(vec![KError::Other(span, err)]),
                    None => return Err(vec![KError::Other(tokens.span(), Error::ExpectedCommaOrRBracket { ctx_span: start_span })]),
            ...
            _ => return Err(vec![KError::Other(span, Error::ExpectedCommaOrRBracket { ctx_span: start_span })]),
        }
    }

    // this section of code can only be reached when the tuple is never terminated with `)`
    Err(vec![KError::Other(tokens.span(), Error::UnclosedBrackets { ctx_span: start_span })])
}

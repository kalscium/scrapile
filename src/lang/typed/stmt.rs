...
pub fn wrap_stmt(stmt: Spanned<Stmt>, type_table: &TypeTable, func_table: &FuncTable, var_table: &mut VarTable) -> Result<Typed<TStmt>, Error> {
    ...
    match stmt {
        ...
        Stmt::VarMutateMod { ident, value } => {
            ...
            // apply the operation
            let value = (TExpr::Mod(Box::new(((TExpr::VarGet { ident: var_ident.clone(), var_type: value.1.clone() }, value.0.1.clone()), value.1.clone())), Box::new(value)), var_type);
